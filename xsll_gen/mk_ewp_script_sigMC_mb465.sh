...
set OUT = "script_mb465/${FLAG}_${FILE}_m${MODE}m_${LEVEL}${TAG}.sh"      
...
set FACTOR = `grep $NAME_MODE ~/script/scale_sigmc_factor_mb465.txt | cut -f 2`
set FACTOR = `grep $NAME_MODE ~/script/scale_sigmc_factor_for_correction_function_mb465.txt | cut -f 2`
