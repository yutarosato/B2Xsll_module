...
namespace Belle {
  ...
  const int Xsd1_LUND      =  30344; // added for xs-spin1 @20130408
  const int antiXsd1_LUND  = -30344; // added for xs-spin1 @20130408
  const int Xsu1_LUND      =  30354; // added for xs-spin1 @20130408
  const int antiXsu1_LUND  = -30354; // added for xs-spin1 @20130408
  ...
  public:
    ...
    std::set<int> Xs_set;   // Xs particles
                            ...
                            // (K0  anti-K0  K+  K-)
                            // (K*0 anti-K*0 K*+ K*-)
    int    gen_fl_xs; // Xs -> +1, other -> -1
    double gen_m_llg;
    double gen_m_ll;  // added at 20120725
    double gen_m_xs;
    double gen_coslp; // added at 20120725
  ...
  protected:
    ...
    int Gen_info( std::map<int, int>& child_id_map, std::multiset<int>& n_particle_set,
    ...
    HepSymMatrix calib_trk_err( Particle p, double scale=1.0 ); // Util.cc
    HepSymMatrix calib_trk_err2( Particle p, double scale=1.0 ); // Util.cc
  };
#if defined(BELLE_NAMESPACE)
}
#endif
