	...
	for(std::vector<Particle>::iterator m = lep_list.begin();
	    ...
	    m++)
	  {
	    ...
	    if( p->lund()*m->lund()==-121 ){
	      if( (pseudo_cc.mass() > PDG_JPsiMass  - 0.39) && (pseudo_cc.mass() < PDG_JPsiMass  + 0.14) ) continue;
	      if( (pseudo_cc.mass() > PDG_Psi2SMass - 0.24) && (pseudo_cc.mass() < PDG_Psi2SMass + 0.09) ) continue;
	    }else if( p->lund()*m->lund()==-169 ){
	      if( (pseudo_cc.mass() > PDG_JPsiMass  - 0.24) && (pseudo_cc.mass() < PDG_JPsiMass  + 0.09) ) continue;
	      if( (pseudo_cc.mass() > PDG_Psi2SMass - 0.14) && (pseudo_cc.mass() < PDG_Psi2SMass + 0.09) ) continue;
	    }
