...
namespace Belle{
  ...
  void XSLL::Mu_cand( std::vector<Particle>& mu_list, const bool fl_message )
  {
	...
	// muid
	Muid_mdst muid( *c );
	//if( muid.Chi_2() <= 0 ) continue;                   // REMOVE FOR FAKE LEPTON !!!!!!!!!!!!!!!!!!
	//if( muid.Muon_likelihood() <= muLH_cut ) continue;  // REMOVE FOR FAKE LEPTON !!!!!!!!!!!!!!!!!!
	...
	if( muid.Chi_2() <= 0 ) info.muonLikelihood( -1 );                     // modified for fake letpon modules at 20120613
	else                    info.muonLikelihood( muid.Muon_likelihood() ); // modified for fake letpon modules at 20120613
