...
namespace Belle{
		       ...
		       BelleTuple* hist, int gen_b_decay_info[][30], const bool fl_message )
  {
	...
	// The definition of coslp is reversed at 20120411
	/*
	...
	*/
	...
	if( err_b==0 ){ // success.
	  if( kf_b.cl()<1.0e-18 ) continue; // added at 20120615 for data suppression
	  hist->column( "kfbcl",  kf_b.cl()    );
	  hist->column( "kfbchi", kf_b.chisq() );
	  hist->column( "kfbdgf", kf_b.dgf()   );
	}else{ // false.
	  continue; // added at 20120615 for data suppression
	  //hist->column( "kfbcl",  -1  );
