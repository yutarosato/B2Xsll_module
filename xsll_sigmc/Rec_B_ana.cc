...
namespace Belle{
			...
			BelleTuple* hist, const bool fl_message )
  {
	...
	if( CC.child(0).genHepevt() && CC.child(0).genHepevt().mother() &&
	  ...
	  hist->column( "gm_ccng", truenchild-2 ); // # of generated bremsstrahlung gamma // "truenchild-3" is correct in signal MC, therefore current "gm_ccng" is # of generated bremsstrahlung gamma + 1. 
	  double e_tot_gam = 0;
