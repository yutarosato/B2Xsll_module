...
// ----------------------------------------------------------------------
ksfwmoments_xsll::ksfwmoments_xsll(const Particle &cand, double ebeam,
				   const Hep3Vector &cmboost, const bool fl_final_xs, const bool fl_message)
{
  if (! IpProfile::usable()) {
    printf("======================================================\n");
  ...
  if( fl_message ) std::cout << "------------------------------------------" << std::endl
			     << "        ksfwmoments_xsll check            " << std::endl
			     << "------------------------------------------" << std::endl;
  //=====================
  ...
  */
  // modified for my xsll modules
  if( fl_final_xs ){ // [signal] l+, l-, k's, pi's
    for( int j=0; j<2; j++ ){
      for (int i = 0; i < cand.child(j).nChildren(); i++){
	const Particle & dau(cand.child(j).child(i));
	if( fl_message ) std::cout << "[ signal sige ] " << dau.pType().lund() << " (" << dau.pType().name() << ")" << std::endl;
	HepLorentzVector p_cms(dau.p());
	p_cms.boost(cmboost);
	p_cms_sigA.push_back(p_cms.vect());
	Q_sigA.push_back((int)dau.charge());
    ...
    for (int i = 0; i < cand.child(0).nChildren(); i++){ // l+ and l-
      const Particle & dau(cand.child(0).child(i));
      if( fl_message ) std::cout << "[ signal sige ] " << dau.pType().lund() << " (" << dau.pType().name() << ")" << std::endl;
      HepLorentzVector p_cms(dau.p());
      p_cms.boost(cmboost);
      p_cms_sigA.push_back(p_cms.vect());
    ...
    const Particle & dau(cand.child(1));
    if( fl_message ) std::cout << "[ signal sige ] " << dau.pType().lund() << " (" << dau.pType().name() << ")" << std::endl;
    HepLorentzVector p_cms(dau.p());
    p_cms.boost(cmboost);
    p_cms_sigA.push_back(p_cms.vect());
  ...
  for (std::vector<Mdst_vee2>::iterator i = veeMgr.begin();
    ...
    if( fl_message ) std::cout << "[other side] " << "Ks" << std::endl;
    Q_other.push_back(0);
  ...
  for (std::vector<Mdst_charged>::const_iterator i = chMgr.begin();
    ...
    if (sig){
      ...
      if (p_cms.rho() > P_MAX) continue;
      p_cms_other.push_back(p_cms.vect());
      if( fl_message ) std::cout << "[other side] " << "Charged(Q=" << (*i).charge() << ")" << std::endl;
  ...
  for (std::vector<Mdst_gamma>::const_iterator i = gMgr.begin();
    ...
    if (sig){
      ...
      if (p_cms.rho() > P_MAX) continue;
      p_cms_other.push_back(p_cms.vect());
      if( fl_message ) std::cout << "[other side] " << "Gamma" << std::endl;
      Q_other.push_back(0);
      p_cms_missA -= p_cms;
      p_cms_missB -= p_cms;
      m_et[0] += p_cms.perp();
  ...
  if( fl_message ){
    std::cout << "p_cms_sigA.size()  = " << p_cms_sigA.size()  << std::endl;
    std::cout << "p_cms_sigB.size()  = " << p_cms_sigB.size()  << std::endl;
    std::cout << "p_cms_other.size() = " << p_cms_other.size() << std::endl;
  }
