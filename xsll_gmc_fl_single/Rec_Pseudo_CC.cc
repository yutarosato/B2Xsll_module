	...
	for(std::vector<Particle>::iterator m = lep_list.begin();
	    ...
	    m++)
	  {
	    ...
	    if( !((p->lund()*m->lund()==-121) || (p->lund()*m->lund()==-169)) ){
	      ...
	      continue;
	    }else if( p->lund()*m->lund()==-121 ){ // electron mode
	      UserInfo& info_lp   = dynamic_cast<UserInfo&>( p->userInfo() );
	      UserInfo& info_lm   = dynamic_cast<UserInfo&>( m->userInfo() );
	      if( (info_lp.eidProb() < eidprob_cut) && (info_lm.eidProb() < eidprob_cut) ) {
		if( fl_message ) std::cout << " -> no electron candidate : " << info_lp.eidProb() << ", " << info_lm.eidProb() << std::endl;
		continue;
	      }
	    }else if( p->lund()*m->lund()==-169 ){ // muon mode
	      UserInfo& info_lp   = dynamic_cast<UserInfo&>( p->userInfo() );
	      UserInfo& info_lm   = dynamic_cast<UserInfo&>( m->userInfo() );
	      if( (info_lp.muonLikelihood() < muLH_cut) && (info_lm.muonLikelihood() < muLH_cut) ){
		if( fl_message ) std::cout << " -> no muon candidate : " << info_lp.muonLikelihood() << ", " << info_lm.muonLikelihood() << std::endl;
		continue;
	      }
	    }
	    ...
	    if( p->lund()*m->lund()==-121 ){
	      if(
		 ( (pseudo_cc.mass() > PDG_JPsiMass  - 0.39) && (pseudo_cc.mass() < PDG_JPsiMass  + 0.14) )
		 ||
		 ((pseudo_cc.mass() > PDG_Psi2SMass - 0.24) && (pseudo_cc.mass() < PDG_Psi2SMass + 0.09) )
		 ){
		if( fl_message ) std::cout << " -> veto charmonium mass band : " << pseudo_cc.mass() << std::endl;
		continue;
	      }
	    }else if( p->lund()*m->lund()==-169 ){
	      if(
		 ( (pseudo_cc.mass() > PDG_JPsiMass  - 0.24) && (pseudo_cc.mass() < PDG_JPsiMass  + 0.09) )
		 ||
		 ( (pseudo_cc.mass() > PDG_Psi2SMass - 0.14) && (pseudo_cc.mass() < PDG_Psi2SMass + 0.09) )
		 ){
		if( fl_message ) std::cout << " -> veto charmonium mass band : " << pseudo_cc.mass() << std::endl;
		continue;
	      }
