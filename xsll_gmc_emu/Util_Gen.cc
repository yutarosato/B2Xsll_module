...
namespace Belle{
  ...
  int XSLL::search_daughter_CC( Gen_hepevt gen, int& id1, int& id2, bool fl_message, int indent, double threshold ){
	...
	std::cout << std::setw(indent*6) << std::right << " +-> "
      ...
      if( sqrt(daughter.E()*daughter.E()-daughter.PX()*daughter.PX()-daughter.PY()*daughter.PY()-daughter.PZ()*daughter.PZ()) > threshold ){ // inclusive J/psi MC has not mass information in gen_hepevt.
	//if( daughter.M() > threshold ){
	if( daughter.idhep() == JPsi_LUND       || daughter.idhep() == Psi2S_LUND   || daughter.idhep() == Psi3770_LUND
	  ...
	  if( nchildchild > 1 ){
	    int tmp_lund = 1;
	    for( int j=0; j<nchildchild; j++ ){
	      Gen_hepevt& grand_daughter( genMgr( Panther_ID(daughter.daFirst()+j ) ) );
	      if( grand_daughter.idhep() != 22 ){
		tmp_lund *= grand_daughter.idhep();
		if( tmp_lund == 1 ) id1 = grand_daughter.get_ID();
		else                id2 = grand_daughter.get_ID();
	      }
	    }
	    if( tmp_lund == -121 || tmp_lund==-169 ){
	      ...
	      if( fl_message ) std::cout << " --> find CC, but not find CC(->ll) particle -> continue itarations" << std::endl;
	      id1 = 0;
	      id2 = 0;
	      int tmp_code = search_daughter_CC( daughter, id1, id2, fl_message, indent+1 );
	      if( tmp_code ) return tmp_code;
	    }
	  }else{
	    if( fl_message ) std::cout << " --> find CC, but not have more than two childlen -> continue iterations" << std::endl;
	    int tmp_code = search_daughter_CC( daughter, id1, id2, fl_message, indent+1 );
	    if( tmp_code ) return tmp_code;
	  }
	  /*
