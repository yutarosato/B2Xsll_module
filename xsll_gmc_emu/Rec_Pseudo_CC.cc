			   ...
			   )
  {
	...
	for(std::vector<Particle>::iterator m = lep_list.begin();
	    ...
	    m++)
	  {
	    ...
	    if( p->lund()*m->lund() != -143 ){ // e-mu combination
	    ...
	    if( abs(p->lund())==Electron_LUND ){
	      pseudo_cc.relation().append( *p ); // 1st child is always electron.
	      pseudo_cc.relation().append( *m ); // 2nd child is always muon
	    }else{
	    ...
	    info.rec_mode( 100*abs(p->lund())+abs(m->lund()) ); // rec_mode = 100*(l+) + (l-) = 1113(e+mu-) or 1311(mu+e-)
